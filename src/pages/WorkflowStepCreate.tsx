...
import { ArrowLeft, Plus, Save, Settings, Zap, Database, FileText, Cpu, Trash2, Code } from 'lucide-react';
...
const WorkflowStepCreate = () => {
  ...
  useEffect(() => {
    if (rawJsonMode) {
      const jsonFromKeyValues = buildJsonFromKeyValues();
      setRawJsonText(JSON.stringify(jsonFromKeyValues, null, 2));
    }
  }, [rawJsonMode, jsonKeyValues]);
  ...
  const switchToKeyValueMode = () => {
    try {
      ...
      const newKeyValues: JsonKeyValue[] = [];
      ...
      if (newKeyValues.length === 0) {
        newKeyValues.push({ id: '1', key: '', value: '' });
      }
      ...
      toast({
        title: "Invalid JSON",
        description: "Please fix the JSON syntax before switching to key-value mode",
        variant: "destructive",
      });
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let configJson;
      if (rawJsonMode) {
        configJson = JSON.parse(rawJsonText);
      } else {
        configJson = buildJsonFromKeyValues();
      }
          ...
          <Card className="shadow-sm">
            <CardHeader>
              <CardTitle className="flex items-center justify-between text-lg">
                ...
                <div className="flex items-center space-x-2">
                  <Label htmlFor="raw-mode" className="text-sm font-normal">Raw JSON</Label>
                  <Switch
                    id="raw-mode"
                    checked={rawJsonMode}
              ...
              <CardDescription>
                Configure model parameters using {rawJsonMode ? 'raw JSON' : 'key-value pairs'}
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="mt-4 p-3 bg-blue-50 rounded-lg">
                ...
                <pre className="text-xs text-blue-800 overflow-auto max-h-32">
                  {rawJsonMode ? rawJsonText : JSON.stringify(buildJsonFromKeyValues(), null, 2)}
                </pre>
              </div>
            </CardContent>
          </Card>
